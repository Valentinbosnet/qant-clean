...
// Utiliser une syntaxe différente et attendre explicitement params
export async function GET(request: Request, { params }: { params: { id?: string } }) {
  try {
    ...
    // Attendre explicitement params avant d'accéder à id
    ...
    try {
      ...
      if (Array.isArray(tables) && tables.length > 0) {
        ...
        if (Array.isArray(portfolios) && portfolios.length > 0) {
          // Ajouter des valeurs par défaut pour balance et currency si elles n'existent pas
          const portfolio = { ...portfolios[0], balance: 10000, currency: "EUR" } as any
      ...
      try {
        ...
        if (typeof db.portfolio === "object") {
          ...
          if (portfolio) {
            // Ajouter des valeurs par défaut pour balance et currency si elles n'existent pas
            const portfolioWithDefaults = { ...portfolio, balance: 10000, currency: "EUR" } as any
