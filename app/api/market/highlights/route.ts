...
export async function GET(request: NextRequest) {
  try {
    ...
    // Utiliser la clé API côté serveur (sans NEXT_PUBLIC_)
    const apiKey = process.env.ALPHA_VANTAGE_API_KEY

    if (!apiKey) {
      console.warn("Clé API Alpha Vantage non configurée, utilisation des données fictives")
      return NextResponse.json(getMockMarketHighlights(limit))
    }

    // Pour l'instant, utiliser les données fictives car l'implémentation complète
    // nécessiterait plusieurs appels API pour chaque action
    return NextResponse.json(getMockMarketHighlights(limit))
  } catch (error) {
